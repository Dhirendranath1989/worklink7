...
router.put('/change-password', authenticateToken, async (req, res) => {
  try {
    ...
    const isFirstTimeGoogleUser = isGoogleUser && !hasExistingPassword;
    
    // Prevent Google users who already have passwords from changing passwords
    if (isGoogleUser && hasExistingPassword) {
      return res.status(403).json({ message: 'Google users with existing passwords cannot change their password' });
    }
    ...
    if (isFirstTimeGoogleUser) {
      ...
      // Check if username already exists (optional - you might want to allow duplicate usernames)
      // This is a basic check, you might want to implement more sophisticated username validation
    }
    
    // Check current password for non-Google users who already have one
    if (!isGoogleUser && hasExistingPassword) {
      if (!currentPassword) {
    ...
    // Add username for first-time Google users
    if (isFirstTimeGoogleUser && username) {
